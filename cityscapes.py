...
if __name__ == '__main__':
  # for split, coarse in zip(['train_extra'], [True]):
  # for split, coarse in zip(['train', 'valid'], [False, False]):
  # for split, coarse in zip(['train', 'valid', 'test'], [False, False, False]):
  for split, coarse in zip(['valid', 'test'], [False, False, False]):
    ...
    a = CityScapesAssembler(
        ...
        split=split,
        coarse_label=coarse,
        output_fname='/ais/gobi4/mren/data/cityscapes/{}_rerun_256x512.h5'.
        format(split))
    a.assemble()
  pass